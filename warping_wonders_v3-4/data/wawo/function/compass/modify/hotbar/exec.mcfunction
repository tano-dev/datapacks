$execute unless data entity @s Inventory[{Slot:$(slot)b}].components.minecraft:lodestone_tracker.target run return run item modify entity @s hotbar.$(slot) wawo:remove_components

$data modify storage eden:temp compass.data.x set from entity @s Inventory[{Slot:$(slot)b}].components.minecraft:lodestone_tracker.target.pos[0]
$data modify storage eden:temp compass.data.y set from entity @s Inventory[{Slot:$(slot)b}].components.minecraft:lodestone_tracker.target.pos[1]
$data modify storage eden:temp compass.data.z set from entity @s Inventory[{Slot:$(slot)b}].components.minecraft:lodestone_tracker.target.pos[2]
$data modify storage eden:temp compass.data.dimension set from entity @s Inventory[{Slot:$(slot)b}].components.minecraft:lodestone_tracker.target.dimension
...
$item modify entity @s hotbar.$(slot) wawo:compass