...
$execute if data storage eden:settings warping_wonders.clock{mob_teleport:"enabled"} as @e[nbt={leash: {UUID:$(uuid)}}] in $(dimension) run tp @s $(x) $(y_adj) $(z)
...
data remove storage eden:temp clock.tp