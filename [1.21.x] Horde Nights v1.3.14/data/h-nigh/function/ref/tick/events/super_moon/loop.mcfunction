...
#======Spawning======#
execute unless score phantom_spawning h-nigh.config matches 1 run function h-nigh:ref/tick/events/super_moon/mob_spawning

#======Recruit=======#
function h-nigh:ref/tick/events/super_moon/horde_mobs
...
scoreboard players add #daylight_tick h-nigh.status 1
execute if score #daylight_tick h-nigh.status matches 2.. run function h-nigh:ref/tick/events/daylight_cycle
...
execute as @e[tag=h-nigh.mob] if score @s h-nigh.vfx_random matches 30..70 at @s run particle minecraft:crimson_spore ~ ~2 ~
