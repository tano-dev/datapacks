...
execute on controller if predicate nova_structures:forward_key run execute on vehicle as @s[tag=sprinting] run attribute @s flying_speed modifier add nova_structures:sprint_flight_speed 0.35 add_multiplied_base
...
execute as @s[tag=sprinting] run execute on passengers run attribute @s movement_speed modifier add nova_structures:boost_player_fov 0.175 add_multiplied_base
